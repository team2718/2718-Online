...
<div class="mx-auto max-w-5xl px-4 py-8">
	...
	{#if form?.success && form.action === 'fixReport'}
		<div
			class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-700"
		>
	...
	<div class="mb-8">
		<div class="mb-3 flex items-center gap-3">
			...
			<span class="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-600"
				>{data.ghostTeams.length}</span
			>
		</div>
		<p class="mb-4 text-sm text-gray-500">
			Teams created automatically by wrong scans — no TBA metadata. Safe to delete once their
			reports are fixed or removed.
		</p>

		{#if data.ghostTeams.length === 0}
			<div
				class="rounded-xl border-2 border-dashed border-gray-200 bg-white p-8 text-center text-sm text-gray-400"
			>
				No ghost teams.
			</div>
		{:else}
					...
					>
						<div>
							...
							{#if team.reportCount > 0}
								<span
									class="ml-2 rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-bold text-red-600"
								>
									{team.reportCount} report{team.reportCount !== 1 ? 's' : ''} — fix first
								</span>
							{:else}
								<span
									class="ml-2 rounded bg-green-100 px-1.5 py-0.5 text-[10px] font-bold text-green-600"
									>ready to delete</span
								>
							{/if}
						</div>
	...
	<div class="mb-8">
		<div class="mb-3 flex items-center gap-3">
			...
			<span class="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-600"
				>{data.reports.length}</span
			>
			{#if data.reports.filter((r) => r.isGhost).length > 0}
				<span
					class="rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-semibold text-orange-700"
				>
		...
		{:else}
			<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
				<!-- Header -->
				<div
					class="grid grid-cols-[5rem_6rem_1fr_1fr_auto] gap-x-3 border-b border-gray-200 bg-gray-50 px-3 py-2 text-[10px] font-bold tracking-widest text-gray-400 uppercase"
				>
					<span>Match</span>
					<span>Team</span>
					<span>Scout</span>
				...
				{#each sortedReports as report, i}
					<div
						class="grid grid-cols-[5rem_6rem_1fr_1fr_auto] items-center gap-x-3 px-3 py-2 text-sm
					...
					>
						...
						<div>
							...
							{#if report.isGhost}
								<span
									class="ml-1 rounded bg-orange-100 px-1 py-0.5 text-[9px] font-bold tracking-wide text-orange-500 uppercase"
									>ghost</span
								>
							{/if}
						...
						{#if editingId === report.id}
							...
							>
								...
								<input
									...
									min="1"
									max="99999"
									class="w-20 rounded border border-gray-300 px-1.5 py-1 text-xs focus:border-blue-400 focus:ring-1 focus:ring-blue-100 focus:outline-none"
								/>
								<button
									type="submit"
									class="rounded bg-blue-600 px-2 py-1 text-[10px] font-bold text-white hover:bg-blue-700"
									>Save</button
								>
								<button
									type="button"
									onclick={cancelEdit}
									class="rounded bg-gray-100 px-2 py-1 text-[10px] font-bold text-gray-500 hover:bg-gray-200"
									>✕</button
								>
							</form>
						{:else}
