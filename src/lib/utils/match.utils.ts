import type { MatchLike } from '$lib/types';

/**
 * Short badge label: Q36, P3, SF1M2, F1M1.
 */
...
/**
 * Sort key for playoff IDs: semifinals before finals, then by set, then match.
 */
export function playoffKey(id: string): number {
	const f = id.match(/^f(\d+)m(\d+)$/);
	if (f) return 100000 + parseInt(f[1], 10) * 10 + parseInt(f[2], 10);
	const sf = id.match(/^sf(\d+)m(\d+)$/);
	if (sf) return parseInt(sf[1], 10) * 10 + parseInt(sf[2], 10);
	return 99999;
}
